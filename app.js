...
async function app(state, update, view) {
  while (true) {
    ...
    if (input.Choices == "Add City") {
      ...
      const updatedModel = await updateadd(input1.addcity, model); //updating the model for the app
      state = {
        ...state,
        model: updatedModel,
      ...
      const updatedModel = await updaterefresh(input2.select, model); //updating the model for the app
      state = {
        ...state,
        model: updatedModel,
        currentView: view(updatedModel),
